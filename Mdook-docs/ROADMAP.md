...
- [ ] **Poetry/verse handling** — Implement Rule 9.2. Detect verse blocks and preserve line breaks. Still not implemented — no verse-specific rule module exists; `BlockQuote`'s docstring mentions preserving verse line breaks as an aspiration, but nothing detects a verse block as distinct from ordinary block-quoted prose.
...
- [ ] **Glossary processing** — A "Glossary" back-matter section is already recognized and named (`mdook.core.rules.zones`/`semantic.py`'s label matching), but its content renders as plain paragraphs, not structured term→definition pairs. Still open.
- [~] **Bibliography processing** — Citation-to-bibliography *linking* is done (Batch 15, `mdook/core/rules/citations.py`) and back-matter Bibliography/References sections are recognized and named. What's *not* done: rendering back matter into separate files per section (`Bibliography.md`, `Glossary.md`, etc.) — everything currently shares one `99 - Back Matter.md` file. Splitting that out is the remaining scope here.
