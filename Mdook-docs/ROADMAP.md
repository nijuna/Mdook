...
- [x] **Interactive CLI Wizard & Directory Scanner** — Implemented `mdook scan` for publication discovery across directories with fast metadata sniffing (title, author, format, size) rendered in a Rich table or clean JSON. Implemented `mdook interactive` / `mdook -i` terminal wizard for guided publication selection, output structure selection (modular library vs single document), parsing profile, destination folder, and AI structure review, with headless auto-prompt in interactive terminal sessions.
- [x] **Cross-platform application packaging** — Configured PyInstaller specification (`mdook.spec`) building standalone executables for Linux (`dist/mdook`) and Windows (`dist/mdook.exe`). Bundled assets, themes, and offline dependencies. Packaged Linux binary with official FreeDesktop `.desktop` file (`packaging/linux/mdook.desktop`) and rootless installer script (`packaging/linux/install-desktop.sh`).
- [x] **Documentation screenshots & visual polish** — Implemented automated headless screenshot capture pipeline (`scripts/capture_screenshots.py`) capturing high-resolution window screenshots across all 3 theme families (*The Library*, *Amethyst*, *Carbon*) in dark/light modes, as well as the modal `SettingsDialog`. Embedded visual assets into `README.md`.
- [x] **Publish Mdook V2 on GitHub** — Finalized release notes covering multi-format support (PDF, EPUB3, DOCX), dual output modes (chapter-split modular libraries and single continuous documents), AI outline review, interactive terminal wizard, and typography-first GUI/CLI. Bumped version to `2.0.0`, tagged release `v2.0.0` on GitHub, and froze codebase.
